...
void updateMVP()
{
	...
	glm::mat4 model = glm::translate(glm::rotate(glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f, 1.0f)), glm::radians(10.0f), glm::vec3(0.0f, 0.0f, 1.0f)), glm::vec3(0.0f, 0.0, 0.0f));
...
void recordCmdBuffer(VkCommandBuffer cmdBuffer, uint32_t ImageIndex)
{
	...
	vkBeginCommandBuffer(cmdBuffer, &cbbi);
	{
		...
		if (hiddenCursor == 0)
			colorValue = { 0.25f, (float)glm::sin(glfwGetTime() / 5), 0.38f, 1.0f };
		else 
